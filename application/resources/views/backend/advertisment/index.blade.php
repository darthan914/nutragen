...
<script type="text/javascript">
	$(function() {
	    ...
	    $('#datatable').on('click', '.delete-advertisment', function(){
			$('#delete-advertisment input[name=id]').val($(this).data('id'));
		});
		$('#datatable').on('click', '.publish-advertisment', function(){
			$('#publish-advertisment input[name=id]').val($(this).data('id'));
		});
		$('#datatable').on('click', '.unpublish-advertisment', function(){
			$('#unpublish-advertisment input[name=id]').val($(this).data('id'));
		});
	});
</script>
@endsection
...
@section('content')
	...
	{{-- Delete advertisment --}}
	<div id="delete-advertisment" class="modal fade" advertisment="dialog">
	...
	@can('publish-advertisment')
	{{-- Read advertisment --}}
	<div id="publish-advertisment" class="modal fade" advertisment="dialog">
		<div class="modal-dialog">
			<div class="modal-content">
				<form class="form-horizontal form-label-left" action="{{ route('backend.advertisment.publish') }}" method="post" enctype="multipart/form-data">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
	...
	{{-- Unpublish advertisment --}}
	<div id="unpublish-advertisment" class="modal fade" advertisment="dialog">
		<div class="modal-dialog">
			<div class="modal-content">
				<form class="form-horizontal form-label-left" action="{{ route('backend.advertisment.publish') }}" method="post" enctype="multipart/form-data">
					<div class="modal-header">
	...
	<div class="x_panel" style="overflow: auto;">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<form method="post" id="action" action="{{ route('backend.advertisment.action') }}" class="form-inline text-right">
					@can('create-advertisment')
					<a href="{{ route('backend.advertisment.create') }}" class="btn btn-default">Create</a>
					@endcan
